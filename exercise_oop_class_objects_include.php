	...
	function displayRate($average){
		if ($average < 80) {
			...
			if ($average >=80 && $average <86){
				return "<font color='Blue'> - Passed-A" . "</font>";
			}
			else {
				return "<font color='Blue'> - Passed-B". "</font>";
			}
		}
		else if ($average >= 90 && $average < 96){
			return "<font color='Green'> - Exceeds Expectations" . "</font>";
		}
		else {
			return "<font color='Green'> - Outsanding". "</font>";
		}
	}
}
Class StudentHS extends Student{
	public function computeAverage(){
		$this->gradeAverage = ($this->subjectGrade1 + 
		$this->subjectGrade2 + $this->subjectGrade3 + 
		$this->subjectGrade4)/4;
